-- ============================================================
-- V2: cruza matrículas com messages_logs (não mais com leads).
-- leads.assigned_to_id quase nunca é preenchido na prática (só 3 de
-- ~770 mil linhas), então não serve como fonte de "quem atendeu".
-- messages_logs.agent_name é a fonte confiável já usada em todo o
-- dashboard de qualidade/pontuação. contact vem como "Nome (telefone)",
-- então extraímos só o nome antes de normalizar.
-- ============================================================
...
AS $$
  ...
  classified AS (
    ...
    LEFT JOIN LATERAL (
      SELECT agent_name, primeiro_atendimento
      FROM contatos ct
