-- ============================================================
-- V4: adiciona janela de validade de 30 dias (atendimento mais
-- antigo que isso não conta — evita atribuir matrícula a um
-- agente cujo único registro é uma conversa velha demais, ex:
-- agente que já não trabalha mais aquele contato) e retorna o
-- detalhamento (aluno + curso) por matrícula válida, pra permitir
-- drill-down na tela.
-- ============================================================
...
AS $$
  ...
  classified AS MATERIALIZED (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE ct.norm_contact = c.norm_aluno
        ...
        AND c.data_matricula - ct.primeiro_atendimento::date <= 30  -- janela de validade
  ...
  validos AS MATERIALIZED (
    SELECT DISTINCT ON (agent_name, norm_aluno, norm_curso)
      agent_name, norm_aluno, norm_curso, aluno, nome_curso, data_matricula
    FROM classified
    WHERE status = 'valido'
