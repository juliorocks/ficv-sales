...
import { cpfDigits, sponteAlunoByCpf } from "./sponte.ts";
...
export async function findAlunoByCpf(db: any, cpf: string) {
    ...
    return (await db.from("alunos").select("id, nome, email, cpf, sponte_aluno_id").in("cpf", [d, fmtCpf(d)]).limit(1).maybeSingle()).data;
}
